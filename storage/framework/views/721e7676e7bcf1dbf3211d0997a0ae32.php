        ...
        <div class="flex flex-col lg:flex-row gap-8 mb-8">
            ...
            <div class="w-full lg:w-2/3" 
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <a href="<?php echo e(route('items.index')); ?>" class="block transition-transform hover:scale-[1.02] bg-emerald-500 p-6 rounded-[2rem] shadow-xl border border-emerald-400/20">
                    ...
                    <a href="<?php echo e(route('admin.claims.index')); ?>" class="block transition-transform hover:scale-[1.02] bg-amber-400 p-6 rounded-[2rem] shadow-xl border border-amber-300/20">
                    ...
                    <a href="<?php echo e(route('admin.claims.history')); ?>" class="block transition-transform hover:scale-[1.02] bg-blue-500 p-6 rounded-[2rem] shadow-xl border border-blue-400/20">
                        <div class="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center text-white mb-4">
                    ...
                    <a href="<?php echo e(route('admin.claims.history')); ?>" class="block transition-transform hover:scale-[1.02] bg-red-500 p-6 rounded-[2rem] shadow-xl border border-red-400/20">
                        <div class="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center text-white mb-4">
                            <i class="fas fa-handshake text-lg"></i>
                        </div>
                    ...
                    <a href="<?php echo e(route('admin.users.index')); ?>" class="block transition-transform hover:scale-[1.02] bg-indigo-500 p-6 rounded-[2rem] shadow-xl border border-indigo-400/20">
                        <div class="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center text-white mb-4">
        ...
        <div class="mb-12 bg-white dark:bg-gray-800 rounded-[2.5rem] p-8 border border-slate-100 dark:border-gray-700 shadow-xl">
            <div class="flex items-center justify-between mb-6">
                ...
                <span class="px-3 py-1 bg-slate-100 dark:bg-gray-700 text-slate-500 rounded-lg text-[10px] font-black uppercase tracking-wider">
                    <?php echo e(auth()->user()->notifications->count()); ?> Total
                </span>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                <?php $__empty_1 = true; $__currentLoopData = auth()->user()->notifications->take(6); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $notification): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <div class="p-5 rounded-3xl <?php echo e($notification->read_at ? 'bg-slate-50 dark:bg-gray-900/50 opacity-60' : 'bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800'); ?> transition-all">
                        ...
                        <p class="text-xs font-bold text-slate-800 dark:text-slate-200 leading-relaxed">
                            <?php echo e($notification->data['message'] ?? 'A claim was updated.'); ?>
                        ...
                        <div class="mt-4 flex items-center justify-between">
                            <span class="text-[10px] px-2 py-0.5 bg-white dark:bg-gray-800 rounded-md font-mono text-slate-500 border border-slate-100 dark:border-gray-700">
                                ID: #<?php echo e($notification->data['claim_id'] ?? '?'); ?>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12 bg-white dark:bg-gray-800 rounded-[3rem] p-4 shadow-2xl border border-slate-100 dark:border-gray-700 overflow-hidden">
            ...
            <div class="p-8 flex flex-col justify-center">
                ...
                <div class="flex gap-4">
                    <a href="<?php echo e(route('items.create')); ?>" class="flex-1 text-center py-4 bg-indigo-600 text-white font-bold rounded-2xl shadow-lg shadow-indigo-200/50">Report Item</a>
                    <a href="<?php echo e(route('items.index')); ?>" class="flex-1 text-center py-4 bg-slate-100 dark:bg-gray-700 font-bold rounded-2xl">Directory</a>
                </div>
            </div>
        </div>
                    ...
                    <div class="group bg-slate-50 dark:bg-gray-900 rounded-[2rem] overflow-hidden border border-slate-100 dark:border-gray-800 transition-all hover:shadow-xl">
                        <div class="h-48 overflow-hidden relative">
                            <?php if($item->image_path): ?>
                                <img src="<?php echo e(asset('storage/' . $item->image_path)); ?>" class="w-full h-full object-cover group-hover:scale-110 transition-transform">
                            <?php else: ?>
                                <div class="flex h-full items-center justify-center opacity-20"><i class="fas fa-image text-3xl"></i></div>
                            <?php endif; ?>
                            <span class="absolute top-4 left-4 px-3 py-1 text-[9px] font-black uppercase rounded-lg shadow-lg <?php echo e($item->type == 'lost' ? 'bg-rose-500 text-white' : 'bg-emerald-500 text-white'); ?>">
                                <?php echo e($item->type); ?>

                            </span>
                        </div>
                        <div class="p-6">
                            <h4 class="font-bold truncate"><?php echo e($item->item_name); ?></h4>
                            <p class="text-[11px] text-slate-400 mt-2 italic"><i class="fas fa-map-marker-alt text-indigo-500 mr-1"></i> <?php echo e($item->location); ?></p>
                            <div class="mt-6 pt-4 border-t border-slate-200 dark:border-gray-800 flex justify-between items-center">
                                <span class="text-[10px] text-slate-400 font-bold"><?php echo e($item->created_at->diffForHumans()); ?></span>
                                <a href="<?php echo e(route('items.show', $item->id)); ?>" class="text-indigo-600 text-[10px] font-black uppercase text-xs">View Details <i class="fas fa-arrow-right ml-1"></i></a>
                            </div>
                        </div>
                    </div>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                ...
                <?php $__empty_1 = true; $__currentLoopData = $claims; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $claim): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <div class="p-6 rounded-[2rem] bg-slate-50 dark:bg-gray-900 border border-slate-100 dark:border-gray-800 transition-all hover:border-indigo-500/30">
                        <div class="flex justify-between items-start mb-4">
                            <h4 class="font-black text-slate-800 dark:text-white truncate"><?php echo e($claim->item->item_name); ?></h4>
                            <span class="text-[9px] font-black uppercase bg-white dark:bg-gray-800 px-2 py-1 rounded shadow-sm text-slate-400">#<?php echo e($claim->id); ?></span>
                        </div>
                        
                        <?php if($claim->is_resolved): ?>
                            <div class="mt-4 p-4 rounded-2xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800/50">
                                ...
                                <div class="mb-4">
                                    ...
                                    <code class="text-xs font-mono font-bold text-slate-700 dark:text-slate-300 bg-slate-200/50 dark:bg-gray-800 px-2 py-1 rounded">
                                        <?php echo e($claim->claim_reference ?? 'REF-PENDING'); ?>

                                    </code>
                                </div>
                                <a href="<?php echo e(route('claims.ads', $claim->id)); ?>" 
...
<?php $__env->stopPush(); ?>
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/riku/Documents/laravel_poject/lost_n_found/resources/views/dashboard.blade.php ENDPATH**/ ?>