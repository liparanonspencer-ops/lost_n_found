            ...
            @forelse($items as $item)
                <div class="group bg-white dark:bg-gray-800 rounded-3xl overflow-hidden border border-slate-100 dark:border-gray-700 hover:shadow-2xl transition-all duration-300 flex flex-col">
                    ...
                    <div class="relative h-56 w-full overflow-hidden bg-slate-100 dark:bg-gray-700">
                        @if($item->image_path)
                            <img src="/images/{{ basename($item->image_path) }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                        @else
                        ...
                        <div class="absolute top-4 left-4">
                            <span class="px-3 py-1 text-[10px] font-black uppercase rounded-full shadow-sm {{ $item->type == 'lost' ? 'bg-rose-500 text-white' : 'bg-emerald-500 text-white' }}">
                                {{ $item->type }}
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="p-5 flex-grow flex flex-col">
                        ...
                        <a href="{{ route('items.show', $item->id) }}" class="mt-auto w-full inline-flex items-center justify-center px-4 py-3 bg-slate-50 dark:bg-gray-700 text-slate-700 dark:text-slate-200 text-sm font-bold rounded-xl hover:bg-indigo-600 hover:text-white transition-all duration-200">
                            View Details
                        </a>
                    </div>
                </div>
